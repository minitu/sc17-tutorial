    ...
    void receive(std::vector<Particle> incoming) {
      ...
      if (++received == 4) {
        ...
        if (iteration % LB_FREQ == 0) {
          /*
           * Add code here to call AtSync() so that load balancing can be
           * performed by the Charm++ runtime. ResumeFromSync(), which has
           * already been added for you down below, is invoked when load
           * balancing is complete. In our code, it returns back to main
           * to check the iteration number as in the else clause.
           */
          // TODO
        }
        else {
